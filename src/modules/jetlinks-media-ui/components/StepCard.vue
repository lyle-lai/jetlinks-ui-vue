...
import {recommendList} from '../views/Home/typings';
...
const jumpPage = (row: recommendList) => {
  ...
  // console.log(2222,row.params)
  row.onClick ? row.onClick(row) : menuStory.jumpPage(row.linkUrl, {params: {id: ':id'}});
  ;
};
</script>
  ...
  .box-list {
    ...
    .list-item {
      ...
      &:not(:last-child)::after {
        ...
        transform: translateY(-50%);
        content: ' ';
        background: url('../assets/home/arrow-1.png') no-repeat 50%;
      }
    }
  }
}
